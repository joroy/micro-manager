...
int XLedDev::Initialize()
{
    ...
    {
        osMessage.str("");
        osMessage << "<XLedDev::Initialize> CreateProperty(" << sLedDevNameLabel << "=" << XLed::Instance()->GetXLedStr(XLed::XL_WLedDevName + m_nLedDevNumber).c_str() << "), ReturnCode=" << ret;
    ...
    if (strcmp(XLed::Instance()->GetXLedStr(XLed::XL_WLedDevName + m_nLedDevNumber).c_str(), XLed::Instance()->GetXLedStr(XLed::XL_WLedDevName + m_nLedDevNumber).c_str()) != 0)
    {
        ...
        {
            osMessage.str("");
            osMessage << "<XLedDev::Initialize> CreateProperty(" << MM::g_Keyword_Description << XLed::Instance()->GetXLedStr(XLed::XL_WLedDevName + m_nLedDevNumber).c_str() << ") ReturnCode=" << ret;
            this->LogMessage(osMessage.str().c_str());
        }
    ...
    ret = CreateProperty(XLed::Instance()->GetXLedStr(XLed::XL_LedDevNameLabel).c_str(), (const char*)sParm, MM::String, true);
    ...
    ret = CreateProperty(XLed::Instance()->GetXLedStr(XLed::XL_LedStatusLabel).c_str(), (const char*)sParm, MM::Integer, false, pAct);
    ...
    ret = CreateProperty(XLed::Instance()->GetXLedStr(XLed::XL_LedHoursLabel).c_str(), (const char*)sParm, MM::Integer, false);
    ...
    ret = CreateProperty(XLed::Instance()->GetXLedStr(XLed::XL_LedTempLabel).c_str(), (const char*)sParm, MM::Integer, false, pAct);
    ...
    ret = GetLedParmVal(sResp, sParm);
    if (ret != DEVICE_OK) strcpy(sIntensity, "Undefined");
    else
    {
    ...
    {
        osMessage.str("");
        osMessage << "<XLedDev::Initialize> UpdateStatus(); RetuenCode = " << ret;
        this->LogMessage(osMessage.str().c_str());
    }
...
int XLedDev::SetOpen(bool yOpen)
{
    ...
    unsigned char sCmdSet[10] = { 0x6F, 0x6E, 0x3D, 0x31, XLed::XL_TxTerm, 0x00, 0x00, 0x00 };
...
{
    CDeviceUtils::CopyLimitedString(Name, XLed::Instance()->GetXLedStr(XLed::XL_WLedDevName + m_nLedDevNumber).c_str());
}
...
int XLedDev::GetLedParmVal(unsigned char* sResp, char* sParm)
{
    ...
    // fill in the NULL to terniate string
    if (sValue != NULL) 
    {
